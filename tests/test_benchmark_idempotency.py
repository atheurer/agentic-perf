...
@pytest.mark.parametrize(
    ...
    [
        ...
        "mcp_reconnect_same_correlation",
        "two_workers_same_intent",
        "explicit_new_attempt",
        "crash_before_launch_takeover",
        "crash_after_launch_reconcile",
        "cached_success_after_restart",
        "cached_failure_after_restart",
        "external_id_persisted",
    ],
)
async def test_execute_fault_matrix_cases_use_persisted_intent(
    monkeypatch, case: str
) -> None:
    """Execute-level matrix cases all enter through the real MCP tool handler.

    The durable operation fake models the state-store boundary; this keeps
    these tests deterministic while the lower-level trace-store suite covers
    SQLite fencing and restart behavior.
    """
    record = {
        "validation_id": f"val-{case}",
        "run_file": {"benchmark": "demo"},
        "runfile_fingerprint": "fp",
        "harness": "crucible",
        "controller": "controller",
    ...
    monkeypatch.setattr(server_utils, "assert_ticket_active", active_check)
    monkeypatch.setattr(
        benchmark_server, "_validation_records", {record["validation_id"]: record}
    )
    monkeypatch.setattr(
        benchmark_server,
        "_get_validated_runfile",
    ...
    monkeypatch.setattr(
        benchmark_server, "_BenchmarkOperation", lambda *_: ExistingOperation()
    )
    result = await benchmark_server.execute_benchmark(
        "controller", validation_id=record["validation_id"]
    )
    assert '"existing_operation": true' in result
    assert calls == ["acquire", "close"]
