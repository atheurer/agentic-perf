...
MCP tools is each `agents/*/server.py` `@mcp.tool()` declaration plus the
native workspace tools registered by `agents/base.py`. CI compares that source
inventory with `agents/tool_audit_policy.py`; every tool has an explicit
