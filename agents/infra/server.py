...
async def setup_controller_ssh_keys(
    ...
    key to each endpoint. Safe to call multiple times — existing keys are
    deduplicated by comment tag.
    """
    ssh = _get_ssh()
    user = ssh_user
    logger.info(f"[infra] Setting up SSH keys: {controller} -> {endpoints}")
